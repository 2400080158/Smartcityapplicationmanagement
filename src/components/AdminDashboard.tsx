...
export function AdminDashboard({ userRole }: AdminDashboardProps) {
  const [services, setServices] = useState<PublicService[]>(getPublicServices(userRole) as PublicService[]);
  const [infrastructure, setInfrastructure] = useState<Infrastructure[]>(getInfrastructure(userRole) as Infrastructure[]);
  const [amenities, setAmenities] = useState<Amenity[]>(getAmenities(userRole) as Amenity[]);
  const [issues, setIssues] = useState<Issue[]>(getIssues(userRole));
  const [feedback, setFeedback] = useState<Feedback[]>(getFeedback(userRole));
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Select value={issue.status} onValueChange={(value) => {
                        setIssues(issues.map(i => i.id === issue.id ? { ...i, status: value as any } : i));
                      }}>
                        <SelectTrigger className="w-32">
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Select value={item.status} onValueChange={(value) => {
                        setFeedback(feedback.map(f => f.id === item.id ? { ...f, status: value as any } : f));
                      }}>
                        <SelectTrigger className="w-32">
                          <SelectValue />
                        </SelectTrigger>
